        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading ? (
            ...
            balances.map((bal) => {
              ...
              if (typeLabel.toUpperCase().includes("EARNED")) accentColor = "from-emerald-400 to-emerald-500";              const timelineVal = (bal as any).leave_template_timeline ?? (bal as any).timeline ?? "";
                          ...
                          return (
                            <tr key={request.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-900/50 transition-colors">
                              <td className="px-4 py-3.5">
                                <span className="font-extrabold text-slate-900 dark:text-zinc-100">#{request.id}</span>
                                <span className="block text-[11px] text-muted-foreground mt-0.5">{formatDate(request.created_at)}</span>
                              </td>
                              <td className="px-4 py-3.5">
                                <Badge variant="outline" className={cn("font-bold px-2.5 py-0.5 border text-[11px] rounded-md", getLeaveTypeBadgeClass(request.leave_type))}>
