...
export default function ClerkLeaveRequestsPage() {
  ...
  // Metrics Calculations (for all active leave requests)
  ...
  const partialCount = requests.filter((r) => getRequestStatus(r) === "PARTIAL").length;
  ...
  return (
    <div className="space-y-8 p-6 max-w-7xl mx-auto bg-linear-to-b from-zinc-50 to-zinc-100/30 dark:from-zinc-950 dark:to-zinc-900/30 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="relative overflow-hidden border border-zinc-200/80 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-md shadow-xs hover:shadow-md transition-all duration-300 group">
          ...
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
              <Info className="h-3 w-3" />
              Total partially approved leave requests
            </p>
          </CardContent>
        </Card>
      ...
      {/* Tabs Filter & Leave requests list - Forced flex-col layout to stack tabs and cards vertically */}
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b pb-2 gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center border border-zinc-200 dark:border-zinc-800 rounded-xl p-1 bg-white dark:bg-zinc-900 shadow-2xs">
              <Button
                size="sm"
                variant={viewMode === "table" ? "secondary" : "ghost"}
                onClick={() => setViewMode("table")}
                className={`h-7 text-xs px-3 rounded-lg gap-1.5 font-bold ${
                  viewMode === "table" ? "bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300" : "text-slate-500"
                }`}
              >
              ...
              <Button
                ...
                className={`h-7 text-xs px-3 rounded-lg gap-1.5 font-bold ${
                  viewMode === "grid" ? "bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300" : "text-slate-500"
                }`}
              >
          ...
          <TabsContent value={activeTab} className="mt-0 focus-visible:outline-none w-full">
            <AnimatePresence mode="popLayout">
              {filteredRequests.length === 0 ? (
                ...
                <Card className="rounded-2xl border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xs overflow-hidden w-full">
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-zinc-800 font-medium">
                        {filteredRequests.map((request) => {
                          ...
                          return (
                            <tr key={request.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-900/50 transition-colors">
                              <td className="px-4 py-3.5">
                                <div className="flex items-center gap-3">
                                  <div className="h-8 w-8 rounded-lg bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold flex items-center justify-center shrink-0 text-xs">
                                    {initials}
                                  </div>
                                  <div>
                                    <span className="font-extrabold text-slate-900 dark:text-zinc-100 capitalize block leading-tight">
                                      {staffName}
                                    </span>
                              ...
                              <td className="px-4 py-3.5 text-center font-extrabold font-mono text-indigo-600 dark:text-indigo-400">
                                ...
                                {isPending ? (
                                  <div className="flex items-center justify-end gap-2">
                                    <Button
                                      ...
                                      className="h-8 text-xs font-bold rounded-xl gap-1 bg-emerald-600 hover:bg-emerald-700 text-white shadow-2xs"
                                    >
